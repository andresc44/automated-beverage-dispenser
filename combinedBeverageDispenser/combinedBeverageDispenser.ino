...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(EMERGENCY_STOP), switchISR, CHANGE); 
  lcdSetup();
  pumpsSetup();
//  lightsSetup();
  lcd.clear();
...
//  ledFinished(false);
  ...
  else lcdPrint("Hello, Please make a selection"); //might need delay after print statements 
...
void switchISR(){
  stopFlag = !stopFlag; //This will cause all changes in switch to set flag high, rather than toggling between states
}
